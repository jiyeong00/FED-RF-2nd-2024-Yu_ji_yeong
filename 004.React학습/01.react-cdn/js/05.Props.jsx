...
// 자동차 정보
import { carImage, carInfo } from "./car_data";
...
function IntroCar({ brand, modelNum }) {
  // 전달속성값 : brand - 자동차 분류명 / modelNum - 배열 순번
  // props 컴포넌트 호출 시 속성값이 객체로 전달하는것을 받아주는 변수다(변수명은 자유!)
  // 사용법 : props.속성명
  // 주의 : 속성명은 컴포넌트를 호출하는 곳에서 정해진 이름을 똑같이 사용해야함
  // 명시적으로 변수명을 사용하려면 구조분해할당형식을 사용하면 된다. {변수명,변수명.....}
  // 구조분해할당으로 받을 경우의 장점은 특정 속성명을 지정하는 효과가

  // 공통 정보 변수에 담기
  //  -> carInfo 하위 [자동차명] 하위 [순번] -> 최종데이터 선택
...
/* 
    [ 데이터 매칭하기 : 속성세팅을 위한 기초 ]
    1. 자동차 분류명 : carInfo 데이터변수 객체의 속성명
    2. 상세모델 배열 번호 : 선택객체 속성값 하위 배열번호
      -> 위의 두가지 값을 전달하며 상세정보를 보낼 수 있다.
*/
...
function DetailCarInfo({ brand, model, color, opt }) {
  ...
  console.log("CSS옵션 객체:", opt);

  //   객체값 추가는 (객체변수, 속성명=값)
  opt.width = "600px";
  //   실제 CSS변수형 속성명으로 사용해야 효음있음
  return (
    <React.Fragment>
      <h2>
      ...
      {/* 리액트 style 속성의 값으로 객체를 css속성에 맞게 주면 인라인코드로 css를 세팅할 수 있다. */}
    </React.Fragment>
  );
} ////////////DetailCarInfo컴포넌트/////////////
