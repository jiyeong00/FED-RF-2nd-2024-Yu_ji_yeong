...
function MakeImage({ isrc, ialt, itit, icss }) {
  // 만약 속성 중 안보낸 것은 출력되지 않는다.
  return <img src={isrc} alt={ialt} title={itit} style={icss} />;
} /////////////MakeImage 컴포넌트///////////////

// 컴포넌트 내부에서 호출하는 다른 컴포넌트를 보통 "서브컴포넌트"라고 부른다.
// 메인 출력 컴포넌트
function Developer({ isNormal, isrc, ialt, itit }) {
  ...
  //   위에 if문 return에 걸리면 아래쪽 return에는 내려오지 않는다
  return (
    <React.Fragment>
      <MakeDeveloper />
      <MakeImage isrc={isrc} ialt={ialt} itit={itit} />
...
function MakeList({ foodName, movieInfo }) {
  ...
  return (
    <li>
      {
        // 음식데이터가 들어온 경우
        // undefined가 아니면 true이므로 && 뒤 코드 출력
          ...
          <ul>
            {
              ...
              // 배열변수.map() 메서드 사용
              // map메서드는 원래 새로운 배열을 현재 자리에 출력하는 용도임
              // 그러나 리액트는 이것을 변경하여 표현식 안에서 출력 시 태그JSX 형식으로 변환해줌
