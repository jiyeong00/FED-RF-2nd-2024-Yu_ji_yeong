...
******************************************/
  function loadFn() {
    ...
    function goSlide(evt, sts = true) {
      ...
      // evt - 이벤트변수 (PointerEvent)
      // sts - 버튼클릭인지 자동호출인지 구분하는 전달변수
      //  -> true면 버튼 클릭 / flase면 자동호출
      ...
      // ->> sts값이 true냐? 맞으면 버튼 클릭한 것임으로 this 키워드에 의한 클래스 ab2존재여부를 물어라!
      //     false냐? false면 무조건 true값을 할당해라
    ...
    // 이벤트 대상 .evt-cover aside
    const evtCover=qsa('.evt-cover aside');
    // 변경대상 .abtn << 이미 abtn변수에 할당됨

    evtCover.forEach((ele,idx) => {
      // console.log(ele);
      // 1. 이벤트 세팅하기 - mouseover - 버튼 보이기
      ele.onmouseover =()=>{
        abtn[idx].style.display="block";
      };//onmouseover
      
      // 2. 이벤트 세팅하기 - mouseout - 버튼 숨기기
      ele.onmouseout =()=>{
