...
// 3. 초기화 작업 : 처음배열 출력 / 콤보박스 바인딩 <<데이터를 콤보박스에 묶어줌
// 3-1. 처음배열 출력
// fruit배열 변경 시 다시 출력해야하므로 함수로 만들기
const showArray = () => {
  showit.innerText = fruit.join("★");
  // 배열.join(구분자) : 배열값 사이에 구분자를 넣은 문자열값 변환
}; ///////showArray

// 처음배열출력함수 최초호출은 아래쪽에서
showArray();
...
// let newArr = newFruits.map(v=>{return `<option>${v}</option>`});
// -> 화살표함수에서 중괄호를 사용하면 return할 경우 return키워드를 써줘야한다.
//    따라서 중괄호생략 시 리턴 키워드 자동생략 문법을 사용하면 간단히 표현할 수 있다.
