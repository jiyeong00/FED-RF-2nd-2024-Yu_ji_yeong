...
/*************************************************************** 
    ...
    (1) 장점: 
        ...
        쿠키와 같이 사용자 동의를 필요로 하는 로컬 기록수단의 단점은 이것을 거부하면 쓸 수 없는데 JS 스토리지는 이런제약이 없음 
         >>>> 보안상 안전이 보장되므로 사용가능함
...
function localsFn() {
  ...
  if (btxt == "처음") {
    ...
    localStorage.setItem("actor-cat", "조직내 스파이 색출하는 해외팀 안기부장");
  } ////if
  else if (btxt == "보여줘") {
...
function makeObj() {
  ...
  // 만약 배열데이터를 직접 넣으려고 하면 로컬쓰는 문자형만 받기 때문에
  // 데이터형 이름만 문자형으로 데이터를 대신 넣게됨
  // >>>>> 배열데이터는 못들어감
  // 넣어야 로컬쓰에 들어간다.
...
// [ 2. 세션 스토리지 연습 ]
// 1. 버튼기능이벤트 대상 : .session-box button
