...
function loadFn() {
  ...
  //   ->> 마지막 번호 뒤는 첫번호, 첫번호 전은 끝번호
  // (4) 해당 순번의 슬라이드에 .on 넣기
  //  ->> 나머지 슬라이드는 on 제거하기(외부함수구성)
  // (5) 블릿표시자도 슬라이드가 같은 순번에 클래스 on넣고 나머지는 빼준다(외부함수구성)
  // (6) 자동넘ㅁ김 구성함수를 호출하여 인터발호출작동
  //  -> 버튼 클릭 시 인터발 지우기 / 일정 시간 뒤 다시 인터발 작동
  ...
  function setClass(target, className, seq) {
    // target - 변경할 요소대상 / className - 변경할 클래스명
    console.log("대상 : ", target, "클래스명 : ", className, "순번 : ", seq);

    // 1. 타켓은 html컬렉션이므로 forEach매서드로 순회함
